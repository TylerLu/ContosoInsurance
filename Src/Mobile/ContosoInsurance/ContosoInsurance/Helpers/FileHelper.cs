...
namespace ContosoInsurance
{
    public class FileHelper
    {
        ...
        public static async Task<string> CopyFileAsync(string itemId, string filename, string filePath, string dataFilesPath)
        {
            var sourceFile = await FileSystem.Current.LocalStorage.GetFileAsync(filePath);
            var sourceStream = await sourceFile.OpenAsync(FileAccess.Read);
            
            return await SaveStreamAsync(itemId, filename, sourceStream, dataFilesPath);
        }
